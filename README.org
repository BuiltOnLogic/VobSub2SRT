...
*** Current state:
   ...
   - Only produces blank image dumps. Cause is known and work is underway.


*** Branch notes:
*legacy*: For compatibility with CMake versions below 4.0

*modern*: CMake version 4.0 drops support for cmake_minimum_required 3.5 or lower.
          Minimum required adjusted to 3.5.

**legacy: (note: may be broken as well. last time I tested it, it did not work. will come back to this soon.)
* Tested with:
...
- GCC 4.4.5
- CMake 2.8.2
- libtiff4-3.9.45squeeze10
- libjpeg62-dev 6b1-1
- tesseract-ocr-dev 2.04.2
- libleptonica-dev 1.62.1.1+b1
- tesseract-ocr-eng

**modern 
Builds but does not produce text, in the middle of reworking code.
Using:

- Arch Linux
- kernel 6.19-rc7 (amd64, custom tuning)
- GCC 15.2.1
- CMake 4.2.3
- libtiff 4.7.1-1
- libjpeg-turbo 3.1.3.1
...
* Building
You will need Tesseract, CMake 3.5+ and GCC 4.3+(debatable...) to build it, as well as a few imaging libraries.
On a Debian-based system you can install the dependencies with:

Note: Substitute libtiff-dev with libtiff4-dev if the package is not found!

#+BEGIN_EXAMPLE
  sudo apt-get install tesseract-ocr-dev libtiff-dev tesseract-ocr-eng build-essential cmake pkg-config
#+END_EXAMPLE
...
For Debian you need the following extra packages, save binutils-gold for a possible edge scenario:

#+BEGIN_EXAMPLE
  sudo apt-get install libpng12-dev libwebp-dev libgif-dev zlib1g-dev libjpeg-dev binutils-gold
#+END_EXAMPLE
...
mplayer as well and link to the bug report.
...
* To Do
- implement preprocessing (first step scaling. Code available in =spudec.c=) (Under investigation@2026-02-24)
